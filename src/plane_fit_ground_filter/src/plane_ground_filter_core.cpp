...
PlaneGroundFilter::PlaneGroundFilter(ros::NodeHandle &nh)
{
    ...
    sub_point_cloud_ = nh.subscribe("/zed2/point_cloud/cloud_registered", 10, &PlaneGroundFilter::point_cb, this); // 输入topoc   /velodyne_points   OR  /kitti/velo/pointcloud
...
void PlaneGroundFilter::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    for (int i = 0; i < laserCloudIn.points.size(); i++)
    {
        ...
        if (laserCloudIn.points[i].z < -0.5 * sensor_height_)
        {
            it++;
        }
        else
