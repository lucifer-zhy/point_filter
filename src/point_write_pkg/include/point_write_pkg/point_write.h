...
public:
    ...
    void run(ros::NodeHandle nh) {
        std::cout << "init!!!" << std::endl;
        // 订阅地图话题，指定回调函数
        map_sub = nh.subscribe<nav_msgs::OccupancyGrid>("/map", 1, &Point_write::mapCallback, this);

        // 创建ROS发布器并声明在全局范围内，发布processed_map地图话题
        map_pub = nh.advertise<nav_msgs::OccupancyGrid>("/processed_map", 1);

        // 订阅 ground_truth 话题
        ros::Subscriber sub_ground_truth = nh.subscribe<nav_msgs::Odometry>("/ground_truth", 1, &Point_write::groundTruthCallback, this);

        // 订阅指定的点云话题
        ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/points_no_ground", 1, &Point_write::pointCloudCallback, this);
    }
    // 订阅groundTruth时的回调函数
    void groundTruthCallback(const nav_msgs::Odometry::ConstPtr& ground_truth) {
        // 从 ground_truth 消息中获取小车的位置信息 -17、7.61小车初始的坐标
